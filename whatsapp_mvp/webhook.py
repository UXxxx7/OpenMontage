...
def _enqueue_revise_style(job_id: str, text: str) -> None:
    from .worker import revise_style

    revise_style(job_id, text)
...
async def assign_endpoint(video_count: int = Form(...), notes: str = Form("")):
    ...
    try:
        ...
        raw, _usage = call_llm_chat(_ASSIGN_SYSTEM, user_msg, temperature=0.0)
        if raw:
            import re as _re
...
def _clips_summary(job) -> list:
    ...
    其余地方"如实告知有哪些没成"的一贯做法一致（degraded_operations/
    quality_warnings 同理），Node 侧靠 error_message 判断要不要显示"第 N
    条失败了"而不是假装那条从来没存在过。"""
    config = get_config()
    out = []
...
async def get_job_endpoint(job_id: str):
    ...
    return {
        ...
        # 当前处理阶段短代码（"planning"/"layout_retry"/"qa_retry"/"rendering"）。
        # Node 网关轮询时把代码翻译成双语文案发给用户，让用户知道管线正在
        # 做什么，不再只有一句笼统的"还在处理"心跳。
        "progress_stage": job.progress_stage,
        # LLM token 用量 + 折算成本（内容规划 + 视觉复审等所有 LLM 调用累计），
        # 跟 generation_cost_usd 一起构成这条视频的真实总花费。
        "llm_tokens_input": job.llm_tokens_input or 0,
        "llm_tokens_output": job.llm_tokens_output or 0,
        "llm_cost_usd": job.llm_cost_usd or 0.0,
        # 管线实际耗时（秒）——跟花费一起如实告知用户这条视频花了多久剪辑。
        "elapsed_seconds": job.elapsed_seconds or 0.0,
        # 视觉复审重试后仍未解决、但没阻断渲染的质量提示（一句话描述 x N）。
        # 现在带瑕疵也照常交付品牌模板版本（用户明确要求），这个字段确保
        # "有瑕疵"这件事仍然被如实告知，不是假装完全没事。
        "quality_warnings": json.loads(job.quality_warnings) if job.quality_warnings else [],
        ...
        # 之前一直没往外暴露过（确认过：这个字段在 API 响应里全程缺失），
        # Node 侧要靠它判断 CLIPS_READY 时该不该走 clip 专属的展示逻辑。
        "pipeline": job.pipeline,
        "clips": _clips_summary(job) if job.pipeline == "clip-factory" else [],
        # 是否正在跑"只重跑样式"的预览修订（_style_revision.json 标记文件是否
        # 存在）。Node 网关靠它挡并发：同一个 job 上第二条修改意见如果在第一条
        # 还没跑完时到达，就提示"还在处理上一条"，不再触发第二次并发重跑
        # （没有 DB 字段的轻量方案，标记文件由 /revise_style 端点同步写入）。
        "style_revision_in_flight": (job.job_dir / "_style_revision.json").exists(),
        # 这个 job 是否被预览编辑器手动改过（render_props_directly 保存成功
        # 后写的标记）。Node 网关靠它决定：用户下次在 WhatsApp 发文字反馈时，
        # 走会从转写重新生成一切的快速/完整重规划路径之前，要不要先弹一句
        # "这会覆盖你的手动编辑"警告确认（Stage 7）。
...
@app.post("/jobs/{job_id}/revise_style")
async def revise_style_endpoint(job_id: str, text: str = Form("")):
    """预览阶段用户打字提意见的快速路径：worker.revise_style 内部判断这条
    反馈是不是"只关于模板展示内容"——是就只重跑 apply_style（不碰视频内容/
    时长，比整单重规划快很多），不是就原样退回完整方案重规划。跟 /revise 的
    区别：/revise 总是走完整方案重规划；这个端点会先尝试更快的样式专项路径，
    对调用方（Node 网关）透明——两条路径最终都会落到
    PREVIEW_READY/WAITING_CONFIRMATION/NEEDS_CLARIFICATION/ERROR 之一。
    """
    job = get_job(job_id)
    if job is None:
        raise HTTPException(status_code=404, detail="Job not found")
    # 幂等：已经在跑（这个端点自己、或 /confirm、/retry 触发的管线），直接
    # 返回当前状态——避免 Node 重试或短时间内两条修改意见触发并发重跑。
    if job.status in (JobStatus.RUNNING_PIPELINE, JobStatus.RENDERING):
        return {"job_id": job_id, "status": job.status.value}
    marker = job.job_dir / "_style_revision.json"
    marker.parent.mkdir(parents=True, exist_ok=True)
    marker.write_text(json.dumps({"feedback": text, "started_at": time.time()}), encoding="utf-8")
    # 必须在 _run_in_background 之前同步落地状态：Node 发完 POST 立刻开始轮询
    # GET /jobs/{id}，如果这里还是 PREVIEW_READY，Node 会把"旧预览"误当成
    # "新预览"立刻投递出去（waitForStatus 的目标状态列表本来就包含
    # PREVIEW_READY，好让它能识别"样式重跑成功、还是 PREVIEW_READY"）。同理
    # 清空 progress_stage——上一轮可能停在"rendering"，不清掉的话 Node 第一次
    # 轮询就会立刻播报一句过期的阶段提示。
    update_job_fields(job_id, status=JobStatus.RUNNING_PIPELINE, error_message=None,
                      progress_stage=None)
    _run_in_background(_enqueue_revise_style, job_id, text)
    return {"job_id": job_id, "status": "RUNNING_PIPELINE"}
...
def serve_file(job_id: str, filename: str):
    ...
    # "."——"../.env" 这类值能原样穿过代理。job.job_dir / filename 本身不做
    # 任何包含性检查，之前只是因为没有会主动构造恶意 filename 的调用方而没
    # 暴露；预览编辑器上线后这条路由第一次会被浏览器 fetch 直接命中（props
    # 文件里如果混进攻击者可控内容，这里就是它能读到仓库任意文件的地方），
    # 必须在那之前把这个口子堵上。
