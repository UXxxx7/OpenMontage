...
5. Burn in captions from `words`, grouped into short readable phrases in sync with the spoken words. Keep them legible (safe-area bottom third, high contrast). CAPTION STABILITY (critical, recurring bug): FIRST build a FIXED array of phrase groups by splitting `words` at pauses (gap > ~0.35s between consecutive words) or every ~6-8 words; THEN for each group display its FULL joined text continuously from the group's first-word start frame to its last-word end frame. NEVER recompute the caption each frame by filtering `words` to the one(s) whose [start,end] contains the current instant — that makes the subtitle flicker word-by-word. The visible caption text MUST stay constant within a phrase and change only at phrase boundaries. If you give a phrase a per-word or per-character color/opacity highlight ("karaoke" reveal), it MUST be a pure function of `useCurrentFrame()`/`interpolate()`, recomputed fresh every single frame from `words[i].start`/`end` — e.g. `interpolate(frame, [startFrame, endFrame], [0, 1])` per word. NEVER implement it as a CSS `transition`/`animation` or a `useEffect`-triggered class toggle that plays once when the element mounts: this editor's caption panel lets the user retype a phrase's text in place, which updates `words[]` without changing that word's array position for the common case (a straight rename) — so the DOM node is NOT remounted, any mount-triggered CSS reveal silently never fires again, and the edited text renders instantly in its fully-revealed end state with no sweep, while unedited text (whose reveal already played once, earlier) still looks fine. A frame-driven `interpolate()` has no such failure mode — it looks correct on the very first frame it's ever evaluated, remount or not.
