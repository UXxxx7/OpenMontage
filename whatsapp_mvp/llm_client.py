...
# whatsapp_mvp/authored/scene_author.py imports _post_bounded directly (its
# own comment: "后台线程 + 硬性总耗时上限，不管服务器怎么挤牙膏都不会无限期
# 卡住") — a hard wall-clock ceiling on top of requests' own (unreliable)
# timeout, via a background thread future. Standalone, independent from
# _post_with_retries below (a separately-evolved retry helper already used
# elsewhere in this file) — added only to satisfy that import, not to change
# any existing call path's behavior.
...
def _post_bounded(
    ...
    """hard_deadline_s：不传就用 `_HARD_CALL_DEADLINE_S`——调用方按自己实际
    的合理调用时长传一个更大的值（scene_author.py 现写/修订场景代码是明显
    更重的调用，需要比默认更长的上限）。"""
...
def _normalize_openai_usage(usage: Optional[dict]) -> dict:
    """OpenAI 兼容的 chat/completions 响应里的 usage 对象 -> 统一形状。DeepSeek
    额外带 prompt_cache_hit_tokens/prompt_cache_miss_tokens（命中缓存的输入
    token 单价低 50 倍）；没有这个细分字段的 provider（纯 OpenAI/custom）就
    全部算作 cache-miss——保守方向，宁可算多一点成本，不凭空假设命中了缓存。"""
    usage = usage or {}
...
def call_llm_chat(system_prompt: str, user_message: str, *, temperature: float = 0.1,
                  model: Optional[str] = None, json_mode: bool = True) -> tuple[Optional[str], Optional[dict]]:
    """Send a single-turn system+user chat completion to the configured LLM provider.

    Returns `(content, usage)` — `content` is the raw text, or None if no
    provider is usable or the call failed; `usage` is a normalized dict
    `{"prompt_tokens", "completion_tokens", "cache_hit_tokens"}` (None when
    `content` is None). Callers that don't need cost tracking can ignore the
    second element (`content, _ = call_llm_chat(...)`).
    ...
    if provider == "claude":
        ...
        try:
            return data["content"][0]["text"], normalized_usage
    ...
    if data is None:
        return None, None
    try:
        return data["choices"][0]["message"]["content"], _normalize_openai_usage(data.get("usage"))
    except (KeyError, IndexError, TypeError) as e:
        logger.error(f"{provider} response missing expected shape: {e}")
        return None, None


def call_vision_chat(text_prompt: str, image_paths: list, timeout: int = 90) -> tuple[Optional[str], Optional[dict]]:
    """视觉子能力调用（独立于主 LLM 通道）。

    主规划走 LLM_*（DeepSeek，纯文本模型）；这里走 VISION_LLM_*（如智谱
    GLM-4V）——只在需要"看图"的环节使用（QA stills 复审等）。未配置
    VISION_LLM_API_KEY 时返回 `(None, None)`，调用方按"没有眼睛"跳过，不影响
    主流程。

    图片以 base64 data URL 内联（OpenAI 兼容 content-parts 格式，智谱/
    Gemini/OpenAI 通用），不依赖公网可访问的图床。

    返回 `(content, usage)`——跟 `call_llm_chat` 同一套约定，`usage` 是
