...
SUPPORTED_OPERATIONS = [
    ...
    "apply_style",           # 渲染品牌风格(浮动卡片+字幕+品牌条+进度条) （无参，自带转写+烧字幕，不要跟 add_subtitles 一起用）
]
...
- apply_style: render the video in the studio's signature branded look — floating rounded speaker card (never full-bleed), karaoke-highlighted captions, a brand strip, and a bottom progress bar. This already transcribes and burns captions itself — do NOT combine it with add_subtitles in the same plan (pick one). no params.
    ...
    def _default_plan(self, edit_request: str) -> dict:
        ...
        if any(w in req for w in ("小红书", "xhs", "好看", "精致", "品牌", "style", "brand")):
            if not any(o["type"] == "apply_style" for o in ops):
                ops.append({"type": "apply_style", "description": "渲染品牌风格（浮动卡片+字幕+品牌条）"})

        if not ops:
            ops = [
                {"type": "remove_silences", "description": "去掉停顿使视频更紧凑"},
                {"type": "apply_style", "description": "渲染品牌风格（浮动卡片+字幕+品牌条）"},
            ]
