...
#
# 输出字段名对齐 contracts/render_props.schema.json（契约②）：
# chapters 用 atFrame（不是 at），数据卡数组叫 dataCards（不是 info_cards）。
# 不产出 mode_schedule —— 那是 PostXhs 的 Dominant/Workflow 卡片模式概念，
# Xiaojin 用 `scenes` 数组做卡片位置调度，不吃 mode_schedule；数据卡出现时
# 要不要挪卡片位置，由 P2 的 apply_style 在拼 scenes 时另外决定，不在这里做。
#
# 范围：目前 dataCards 只支持 count-up 数字卡（没有日历/仪表盘组件），所以
...
def plan_content(segments: list[dict], duration: float) -> dict[str, Any]:
    """转写分段 -> {chapters, dataCards}（frame 单位，字段名对齐契约②，可直接
    并入 XiaojinEditorial 的 render_props）。

    LLM 调用失败或没配 key 时，返回空计划（跟 apply_style 最初的行为一致，只是
    "外壳"效果）——内容判断本来就是锦上添花，不应该因为它失败就搞垮整条剪辑流程。
    """
    config = get_config()
    empty = {"chapters": [], "dataCards": []}

    if not config.llm_api_key or not config.llm_base_url:
        logger.info("content_planner: 没配 LLM，跳过内容规划（返回空外壳计划）")
        return empty

    endpoint = config.llm_base_url.rstrip("/")
    ...
    try:
        ...
        logger.warning(f"content_planner: LLM 调用/解析失败，跳过内容规划: {e}")
        return empty
...
def _to_frame_plan(raw: dict, duration: float) -> dict[str, Any]:
    ...
    for dp in raw.get("data_points") or []:
        ...
        for r, sec in zip(rows_in, row_seconds):
            ...
            if value is None:
                continue  # dataCard rows are count-up only; skip anything without a real number
    ...
    return {"chapters": chapters, "dataCards": data_cards}
...
def plan_filler_removal(words: list[dict], duration: float) -> list[dict]:
    ...
    """
    config = get_config()
    if not config.llm_api_key or not config.llm_base_url or not words:
        logger.info("content_planner: 没配 LLM 或没有词级时间戳，跳过口误检测")
        return []
    ...
    try:
        ...
        logger.warning(f"content_planner: 口误检测 LLM 调用/解析失败，跳过: {e}")
        return []
