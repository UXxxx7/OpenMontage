...
# 空的 chapters/infoCards 去渲染。对应该文档的 "Data Display Analysis
...
def plan_content(segments: list[dict], duration: float) -> dict[str, Any]:
    """转写分段 -> 章节 + 数据卡计划（已经是 frame 单位，可以直接喂给 PostXhsEditorial）。

    LLM 调用失败或没配 key 时，返回空计划（跟 apply_style 最初的行为一致，只是
    "外壳"效果）——内容判断本来就是锦上添花，不应该因为它失败就搞垮整条剪辑流程。
    """
    config = get_config()
    empty = {"chapters": [], "info_cards": [], "mode_schedule": [{"frame": 0, "mode": "dominant"}]}
...
def _to_frame_plan(raw: dict, duration: float) -> dict[str, Any]:
    ...
    for c in raw.get("chapters") or []:
        try:
            chapters.append({"at": max(0, round(float(c["at_seconds"]) * FPS)), "label": str(c["label"])[:20]})
    ...
    return {"chapters": chapters, "info_cards": info_cards, "mode_schedule": dedup}
